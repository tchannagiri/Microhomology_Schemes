// 'Alphabet' colors taken from plotly code
...
// function init_region_info() {
//   for (var hguide in REGION_INFO) {
//     var strand = REGION_INFO[hguide].keys()[0];

//     var opp_strand = strand == 'R1' ? 'R2' : 'R1';
//     var ref_length = REGION_INFO[hguide][strand]['ref_seq'].length;
  ...
  for (var micro of microhomologies) {
    for (var pos of ['pos_left', 'pos_right']) {
      seq_svgs.append('rect')
        ...
        .attr('height', seq_height)
        .style('fill', 'transparent')
        // .style('stroke', ALPHABET[micro_idx % ALPHABET.length])
        .style('stroke', 'black')
        .style('stroke-width', 2);
    }
...
function make_schemes(content_selector, hguide, strand, treatment, microhomology_group, combined) {
  ...
  var pcis_height = 20;
  var areas_height = 20;
  // var branch_site_height = 20;
  var seq_height = 20;
  var bars_1_height = 20;
  // var bars_2_height = 20;

  var microhomologies = MICROHOMOLOGIES.filter(
    x => ((x['hguide'] == hguide) && (x['strand'] == strand) && (x['treatment'] == treatment))
  );
  if (microhomology_group != 'all') {
    microhomologies = microhomologies.filter(
      x => x['microhomology_group'] == microhomology_group
    );
  }
  ...
  var areas_y = pcis_y;
  // var branch_site_y = areas_y + areas_height;
  // var seq_y = branch_site_y + branch_site_height;
  ...
  // var bars_2_y = bars_1_y + bars_1_height;
  // var scheme_height = bars_2_y + bars_2_height;
  ...
  for (var area in AREAS[hguide][strand]) {
    if (area == 'intron') {
      continue;
    }

    for (var [start, end] of AREAS[hguide][strand][area]) {
  ...
  if (combined) {
    make_seq_svgs_combined(
      ...
      REF_SEQ[strand],
      microhomologies,
      DELETE_BRANCH[treatment] && AREAS[hguide][strand]['branch'][0],
    );
  } else {
